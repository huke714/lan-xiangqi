...
const ChessGame = (() => {
  ...
  function switchTimer() {
    ...
    if (activeTimerColor === RED) {
      redTime += timeIncrement;
      activeTimerColor = BLACK;
    } else {
  ...
  function canEnterSandbox() {
    ...
    if (!state.myColor || state.currentTurn !== state.myColor) return false;
    if (timeControlSeconds > 0) {
      // 须在对局结束前保留 30 秒，剩余不足则不能进沙盘
  ...
  function applyLiveMoveToSnapshot(from, to) {
    ...
    if (isCheckmate) {
      ...
      if (timeControlSeconds) {
        if (snap.activeTimerColor === RED) {
          snap.redTime += timeIncrement;
          snap.activeTimerColor = BLACK;
        } else {
          snap.blackTime += timeIncrement;
          snap.activeTimerColor = RED;
        }
      }
    }
  }
