...
const App = (() => {
  ...
  function initBoardInteraction() {
    ...
    ChessGame.init({
      ...
      onMoveMade: (from, to) => {
        if (ChessGame.isInSandbox()) {
          MoveListView.rebuild(ChessGame.getSandboxExploreHistory());
          ChessBoard.updateBoardBorder(ChessGame.getState().currentTurn);
          updateSandboxNavUi();
          return;
        }
  ...
  function hideAllViews() {
    ...
    const joinRoom = document.getElementById('joinRoomOverlay');
    ...
    if (joinRoom) joinRoom.classList.remove('active');
  ...
  function renderRoomList(rooms) {
    ...
    container.querySelectorAll('button[data-room-id]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (action === 'spectate') {
          beginJoinRoom(roomId, { asSpectator: true });
        } else {
          beginJoinRoom(roomId, { asSpectator: false });
        }
      });
    });
  }
  ...
  function readNameFromInput(id, fallbackEmptyToRandom) {
    const input = document.getElementById(id);
    const name = input ? input.value.trim() : '';
    if (name) return name.slice(0, 10);
    if (fallbackEmptyToRandom === false) return '';
    return '玩家' + Math.floor(Math.random() * 1000);
  }

  function syncLobbyName(name) {
    const lobby = document.getElementById('playerName');
    if (lobby && name) lobby.value = name;
  }

  function fillNameInputs(preferred) {
    const base = (preferred || '').trim();
    ['createPlayerName', 'joinPlayerName'].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.value = base;
    });
  }
  ...
  function handleCreateRoom() {
    ...
    const lobby = document.getElementById('playerName');
    const lobbyName = lobby ? lobby.value.trim() : '';
    fillNameInputs(lobbyName);
    showView('colorSelect');
    const createName = document.getElementById('createPlayerName');
    if (createName) setTimeout(() => createName.focus(), 50);
  }
  ...
  function beginJoinRoom(roomId, options) {
    const id = String(roomId || '').trim().toUpperCase();
    if (!id) return;
    const opts = options || {};
    // 回到自己的对局：不打断填昵称，直接重连
    const session = SessionStore.read();
    if (!opts.asSpectator && session && session.roomId && String(session.roomId).toUpperCase() === id && session.playerToken) {
      sendJoinRoom(id, opts);
      return;
    }
    pendingAction = 'join';
    pendingRoomId = id;
    pendingJoinOptions = {
      ...
      preferredColor: opts.preferredColor == null ? null : opts.preferredColor,
    };
    const codeEl = document.getElementById('joinRoomCodeDisplay');
    if (codeEl) codeEl.textContent = id;
    const hint = document.getElementById('joinRoomHint');
    if (hint) {
      hint.textContent = opts.asSpectator
        ? '观战可旁观对局，也可在聊天中发言'
        : '加入后将执另一方（先手或后手取决于房主所选）';
    }
    const btn = document.getElementById('btnConfirmJoin');
    if (btn) btn.textContent = opts.asSpectator ? '进入观战' : '进入对局';
    const joinName = document.getElementById('joinPlayerName');
    const lobby = document.getElementById('playerName');
    if (joinName) joinName.value = (lobby && lobby.value.trim()) || '';
    hideColorSelect();
    const overlay = document.getElementById('joinRoomOverlay');
    if (overlay) overlay.classList.add('active');
    if (joinName) setTimeout(() => joinName.focus(), 50);
  }
  ...
  function confirmJoinRoom() {
    ...
    const name = readNameFromInput('joinPlayerName', true);
    myName = name;
    syncLobbyName(name);
    const opts = pendingJoinOptions || { asSpectator: false };
    const roomId = pendingRoomId;
    cancelJoinRoom();
    sendJoinRoom(roomId, opts);
  }
  ...
  function updateSandboxNavUi() {
    const prev = document.getElementById('btnSandboxPrev');
    const next = document.getElementById('btnSandboxNext');
    const inSandbox = ChessGame.isInSandbox();
    if (prev) prev.disabled = !inSandbox || !ChessGame.canSandboxStepBack();
    if (next) next.disabled = !inSandbox || !ChessGame.canSandboxStepForward();
  }

  function sandboxStepBack() {
    if (!ChessGame.isInSandbox()) return;
    if (!ChessGame.sandboxStepBack()) return;
    MoveListView.rebuild(ChessGame.getSandboxExploreHistory());
    updateSandboxNavUi();
    const st = ChessGame.getState();
  ...
  function sandboxStepForward() {
    if (!ChessGame.isInSandbox()) return;
    if (!ChessGame.sandboxStepForward()) return;
    MoveListView.rebuild(ChessGame.getSandboxExploreHistory());
    updateSandboxNavUi();
    const st = ChessGame.getState();
    ChessBoard.updateBoardBorder(st.currentTurn, st.inCheck);
  ...
  function updateSandboxTimeUi(remaining, duration) {
    ...
    if (!el) return;
    el.classList.remove('is-label', 'urgent');
    if (!ChessGame.getTimeControl()) {
      el.textContent = '不限时';
      el.classList.add('is-label');
      return;
    }
    if (ChessGame.isSandboxAwaitingOwnTurn && ChessGame.isSandboxAwaitingOwnTurn()) {
      el.textContent = '等待行棋';
      el.classList.add('is-label');
      return;
    }
  ...
  function selectColor(color) {
    ...
    if (pendingAction === 'create') {
      const typed = readNameFromInput('createPlayerName', false);
      myName = typed || getPlayerName();
      if (typed) syncLobbyName(typed);
